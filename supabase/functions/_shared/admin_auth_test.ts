...
Deno.test("parseAdminPath overview", () => {
  const { route } = parseAdminPath(
    new URL("http://localhost/admin-api/v1/overview"),
  );
...
Deno.test("parseAdminPath user detail", () => {
  const url = new URL(
    "http://x/admin-api/v1/users/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
  );
...
Deno.test("parseAdminPath deployed edge function path", () => {
  ...
  if (session.route !== "session") {
    throw new Error(`session route got ${session.route}`);
  }

  const overview = parseAdminPath(
    new URL("https://ref.supabase.co/functions/v1/admin-api/v1/overview"),
  );
  if (overview.route !== "overview") {
    throw new Error(`overview route got ${overview.route}`);
  }
  ...
  if (userDetail.route !== "users/detail") {
    throw new Error(`got ${userDetail.route}`);
  }
...
Deno.test("parseAdminPath device revoke", () => {
  const parsed = parseAdminPath(
    new URL(
      "http://x/admin-api/v1/devices/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb/revoke",
    ),
  );
...
Deno.test("parseAdminPath entitlement override delete", () => {
  const parsed = parseAdminPath(
    new URL(
      "http://x/admin-api/v1/entitlement-overrides/cccccccc-cccc-cccc-cccc-cccccccccccc",
    ),
  );
...
Deno.test("parseAdminPath manual subscription grant", () => {
  const parsed = parseAdminPath(
    new URL(
      "http://x/admin-api/v1/users/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/subscription",
    ),
  );
  if (parsed.route !== "users/subscription") {
    throw new Error(`got ${parsed.route}`);
  }
  if (parsed.params.userId !== "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa") {
    throw new Error("userId mismatch");
...
Deno.test("parseAdminPath user lifecycle controls", () => {
  for (const action of ["subscription-cancel", "trial-revoke", "delete"]) {
    const parsed = parseAdminPath(
      new URL(`http://x/admin-api/v1/users/user-1/${action}`),
    );
    if (parsed.route !== `users/${action}`) {
      throw new Error(`${action} route got ${parsed.route}`);
    }
    if (parsed.params.userId !== "user-1") {
      throw new Error(`${action} userId mismatch`);
    }
  }
});
