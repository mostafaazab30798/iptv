...
void main() {
  group('PlayerScreen Widget Tests', () {
    ...
    testWidgets('attaches PlayerView after the route owns the surface', (
      tester,
    ) async {
    ...
    testWidgets('displays channel title when playing a live channel', (
      tester,
    ) async {
      await tester.pumpWidget(createTestApp());
      await tester.pump();
    ...
    testWidgets(
      'renders PlayerErrorView on unrecoverable error and handles retry',
      (tester) async {
        await tester.pumpWidget(createTestApp());
        await tester.pump();
    ...
    testWidgets(
      'shows non-blocking reconnecting HUD during auto-reconnect attempts',
      (tester) async {
        await tester.pumpWidget(createTestApp());
        await tester.pump();
    ...
    testWidgets(
      'stops playback and releases resources when PlayerScreen is popped or closed',
      (tester) async {
    ...
    testWidgets('renders Replay 10s and Forward 10s buttons and handles taps', (
      tester,
    ) async {
      await tester.pumpWidget(createTestApp());
      await tester.pump();
    ...
    testWidgets(
      'in Arabic RTL mode, Previous is on left, Next is on right, and taps switch channels correctly',
      (tester) async {
    ...
    testWidgets(
      'lazy live playlist switches channels via next and previous buttons',
      (tester) async {
        await tester.pumpWidget(createTestApp());
        await tester.pump();
    ...
    testWidgets('double-tap on left and right screen edges triggers 10s seek', (
      tester,
    ) async {
      await tester.pumpWidget(createTestApp());
      await tester.pump();
  ...
  testWidgets(
    'seek bar previews while dragging and commits the final position',
    (tester) async {
      ...
      expect(
        errorMilliseconds,
        lessThan(const Duration(seconds: 1).inMilliseconds),
        reason: 'the slider should map touch position accurately to VOD time',
      );
    },
  );
}
